...
# This is the superior PyQt6 implementation""")
    ...
    def create_ui(self):
        ...
        self.sidebar_toggle_action.setChecked(True)  # Sidebar visible by default
    ...
    def load_current_colors_into_dialog(self):
        """Load current colors into customization dialog"""
        if not hasattr(self, 'customization_dialog') or not self.customization_dialog:
            return
    
        # This would load your current colors into the dialog
        # You'll need to track colors in your UI elements
        pass
    ...
    def apply_custom_colors(self, colors, preview=False):
        ...
        # DEBUG: Print what we're trying to apply
        print(f"DEBUG: Applying colors: {list(colors.keys())[:10]}...")
    
        # Apply to sidebar - using object names instead of property selectors
        sidebar_scroll = self.findChild(QScrollArea)
        if sidebar_scroll:
            sidebar_container = sidebar_scroll.widget()
            sidebar = sidebar_container.findChild(QFrame)
            if sidebar:
                # Build sidebar style
                sidebar_style = f"""
                    QFrame {{
                        background-color: {colors.get('sidebar_bg', '#111122')};
                        border-right: 2px solid {colors.get('sidebar_btn_border', '#00ff41')};
                    }}
                """
                sidebar.setStyleSheet(sidebar_style)
    
        # Apply to sidebar widgets by finding them directly
        central = self.centralWidget()
        if central:
            # Find all labels in the sidebar area
            for label in central.findChildren(QLabel):
                label_text = label.text().upper()
                if "LEVIATHAN" in label_text and "EDIT" in label_text:
                    label.setStyleSheet(f"color: {colors.get('sidebar_title', '#00ff41')};")
                elif "ULTIMATE" in label_text:
                    label.setStyleSheet(f"color: {colors.get('sidebar_version', '#ff0066')};")
                elif "CLASSIFIED" in label_text or "DISPLAY" in label_text:
                    label.setStyleSheet(f"color: {colors.get('sidebar_labels', '#00ffaa')};")
        
            # Find all sidebar buttons
            for btn in central.findChildren(QPushButton):
                if btn.parent() and hasattr(btn.parent(), 'styleSheet'):
                    parent_style = btn.parent().styleSheet()
                    if "sidebar" in parent_style.lower():
                        # This is a sidebar button
                        bg_color = colors.get('sidebar_btn_bg', '#002200')
                        text_color = colors.get('sidebar_btn_text', '#00ff41')
                        border_color = colors.get('sidebar_btn_border', '#00ff41')
        ...
        if toolbar:
            toolbar.setStyleSheet(f"""
                QToolBar {{
                    background: {colors.get('toolbar_bg', '#111122')};
                    padding: 5px;
                }}
            """)
            ...
            for action in toolbar.actions():
                if action.text():
                    # Style the associated widget if it's a button
                    for widget in toolbar.children():
                        if hasattr(widget, 'defaultAction') and widget.defaultAction() == action:
                            bg_color = colors.get('toolbar_btn_bg', '#003300')
                            text_color = colors.get('toolbar_btn_text', '#00ff41')
        ...
        if hasattr(self, 'status_bar'):
            bg_color = colors.get('status_bg', '#330000')
            text_color = colors.get('status_text', '#ff0066')
            border_color = colors.get('status_border', '#ff0066')
        ...
        if hasattr(self, 'output_console'):
            bg_color = colors.get('console_bg', 'transparent')
            text_color = colors.get('console_text', '#ff4444')
            border_color = colors.get('console_border', '#ff4444')
        ...
        for key, color_value in colors.items():
            if key.startswith('btn_'):
                self.style_individual_button(key, color_value)
    
        # Apply to editors
        for i in range(self.tabs.count()):
            tab = self.tabs.widget(i)
            if hasattr(tab, 'editor'):
                bg_color = colors.get('editor_bg', 'transparent')
                text_color = colors.get('editor_text', '#00ff41')
                select_color = colors.get('editor_selection', '#003300')
        ...
        if not preview:
            ...
            print("DEBUG: Custom colors applied successfully!")
