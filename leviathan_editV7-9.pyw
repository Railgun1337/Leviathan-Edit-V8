...
class LeviathanEditUltimate(QMainWindow):
    def __init__(self):
        ...
        # Autosave setup - only saves files that have been manually saved before
        self.autosave_timer = QTimer()
        self.autosave_timer.timeout.connect(self.autosave_all_tabs)
        self.autosave_timer.start(60000)  # Autosave every 60 seconds (1 minute)
        self.autosave_enabled = True

        # Session persistence for autosave
        self.session_file = os.path.join(os.path.expanduser('~'), '.leviathan_edit', 'session.json')
        self.load_session()
...
# This is the superior PyQt6 implementation""")
    ...
    def init_themes(self):
        ...
        self.themes["CIA BlackOps"].update({
            ...
            "accent": "#00ffff",
            "sidebar_bg": "#001833",
            "sidebar_title": "#00ffff",
            "sidebar_btn_bg": "#003366"
        })
        
        # Military SFU
        self.themes["Military SFU"] = self.themes["FBI Terminal"].copy()
        self.themes["Military SFU"].update({
            "bg": "#001122", 
            "panel": "#001833", 
            "text": "#00ffff", 
            "accent": "#00ffff",
        ...
        # Cyber-Samurai
        self.themes["Cyber-Samurai"] = self.themes["FBI Terminal"].copy()
        self.themes["Cyber-Samurai"].update({
            "bg": "#0a0f14",  # Darker blue-black base
            "panel": "#1a2430",  # Deep slate with blue undertone
            "text": "#ccd9ff",  # Softer, mist-like blue-white
            "accent": "#66aaff",  # Electric blue samurai glow
            "accent2": "#ff3366",  # Cherry blossom/mono-filament accent
            "sidebar_bg": "#15202b",
            "sidebar_title": "#88aaff",
            "sidebar_btn_bg": "#223344",
            "terminal": "#112233",
            "terminal_text": "#aaddff",
            "glow_effect": True  # Adds subtle edge glow to panels
        })
    ...
    def create_ui(self):
        ...
        autosave_action.setObjectName("btn_autosave")
    ...
    def save_file(self):
        ...
        if hasattr(current_tab, 'file_path') and current_tab.file_path:
            ...
            path = current_tab.file_path
        else:
            # No path yet, ask user
        ...
        try:
            ...
            # Store content for autosave comparison
            current_tab.last_saved_content = content
        
            self.tabs.setTabText(self.tabs.currentIndex(), os.path.basename(path))
            QMessageBox.information(self, "FILE SAVED", f"File saved successfully.\n\nAutosave: {'ENABLED' if self.autosave_enabled else 'DISABLED'}")
    ...
    def autosave_all_tabs(self):
        """Autosave all tabs that have been manually saved before"""
        ...
        for i in range(self.tabs.count()):
            ...
            # Check if this tab has a saved file path
            if not hasattr(tab, 'file_path') or not tab.file_path:
                continue  # Skip untitled files
        
            # Get the current content
            content = tab.editor.toPlainText()
        
            # Check if content has changed since last save
            if hasattr(tab, 'last_saved_content') and tab.last_saved_content == content:
            ...
            try:
                ...
                print(f"Autosave failed for {tab.file_path}: {e}")
    
        if saved_count > 0:
            self.status_bar.showMessage(f"💾 AUTOSAVED {saved_count} file(s)", 3000)
            self.save_session()  # Update session after autosave
    ...
    def toggle_autosave(self):
        ...
        if self.autosave_enabled:
            ...
            self.status_bar.showMessage("✓ AUTOSAVE ENABLED (every 60 seconds)")
        else:
            self.autosave_timer.stop()
            self.status_bar.showMessage("✗ AUTOSAVE DISABLED")
    ...
    def set_autosave_interval(self, seconds):
        ...
        self.status_bar.showMessage(f"AUTOSAVE INTERVAL: {seconds} seconds")

    def save_session(self):
        """Save current session (open files and their paths) to JSON"""
        import json
        import os
        ...
        for i in range(self.tabs.count()):
            ...
            tab_info = {
                ...
                'content': tab.editor.toPlainText(),
                'file_path': getattr(tab, 'file_path', None),  # None if untitled
                'last_saved_content': getattr(tab, 'last_saved_content', None)
            }
        ...
        try:
            ...
            print(f"DEBUG: Session saved with {len(session_data['tabs'])} tabs")
        except Exception as e:
    ...
    def load_session(self):
        ...
        if not os.path.exists(self.session_file):
            print("DEBUG: No session file found, starting fresh")
            return
        ...
        try:
            ...
            print(f"DEBUG: Loading session with {len(session_data['tabs'])} tabs")
        
            # Clear the default tab if it exists and is empty
            if self.tabs.count() == 1:
                first_tab = self.tabs.widget(0)
                if hasattr(first_tab, 'editor'):
                    if not first_tab.editor.toPlainText().strip():
                        self.tabs.removeTab(0)
        
            # Restore each tab
            for tab_info in session_data['tabs']:
                self.create_new_tab(initial_text=False)
            
                current_tab = self.tabs.currentWidget()
                if hasattr(current_tab, 'editor'):
                    # Restore content
                    current_tab.editor.setPlainText(tab_info['content'])
                
                    # Restore file path (critical for autosave!)
                    if tab_info['file_path']:
                        current_tab.file_path = tab_info['file_path']
                        current_tab.last_saved_content = tab_info.get('last_saved_content')
                
                    # Restore tab title
                    self.tabs.setTabText(self.tabs.currentIndex(), tab_info['title'])
        
            # Restore active tab
            if 'current_tab' in session_data:
                self.tabs.setCurrentIndex(session_data['current_tab'])
        
            self.status_bar.showMessage(f"SESSION RESTORED: {len(session_data['tabs'])} tabs loaded")
        
        except Exception as e:
            print(f"ERROR loading session: {e}")
            # If loading fails, just start fresh (don't crash)

    def remember_file_path(self, file_path):
        """Remember that a file has been saved before (permanent history)"""
        import json
        import os
    
        config_dir = os.path.join(os.path.expanduser('~'), '.leviathan_edit')
        history_file = os.path.join(config_dir, 'file_history.json')
        ...
        # Add this file (use absolute path as key)
    ...
    def is_file_known(self, file_path):
        """Check if a file has ever been saved before"""
        import json
        import os
        ...
        try:
            ...
            abs_path = os.path.abspath(file_path)
            return abs_path in history
        except:
            return False

    def closeEvent(self, event):
        """Called when window is closed - save session before exit"""
        # Save session so we can restore next time
        self.save_session()
    
        # Also save custom colors
        if hasattr(self, 'custom_colors'):
            self.save_custom_colors(self.custom_colors)
